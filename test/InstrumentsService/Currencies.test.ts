...
describe('Запрашиваем списки валют', () => {
  test('Получаем (весь) список валют', async () => {
    const response = await instrumentsService.Currencies({
      instrumentStatus: InstrumentStatus.INSTRUMENT_STATUS_ALL
    })
    expect(response).toHaveProperty('instruments')
  })

  test('Получаем (без указания статуса) список валют', async () => {
    const response = await instrumentsService.Currencies({
  ...
  test('Получаем (базовый) список валют', async () => {
    const response = await instrumentsService.Currencies({
  ...
  describe('Запрашиваем конкретную валюту', () => {
    ...
    test('Получаем валюту (указав фиги)', async () => {
      const response = await instrumentsService.CurrencyBy({
    ...
    test('Получаем валюту (указав тикер)', async () => {
      const response = await instrumentsService.CurrencyBy({
    ...
    test('Получаем валюту (указав айди)', async () => {
      const response = await instrumentsService.CurrencyBy({
    ...
    test('Получаем валюту (указав позицию)', async () => {
      const response = await instrumentsService.CurrencyBy({
